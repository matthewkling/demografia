...
# convert an annual simplex to a multiyear simplex
...
# time series population projection
sim <- function(x, p, fecundity, iter = 50){
  ...
  for(i in 1:iter){
    # x <- x %*% t(p)
    xi <- sapply(1:length(x), function(j){
      rmultinom(1, x[j], p[,j])
    }) %>%
      apply(1, sum)
    xi[1] <- x[4] * fecundity
...
# t <- sample(1:25, n_blocks, replace = T)
...
# for FIA, may want to to set one or both of these two params to zero (or another arbitrary value)
...
# rstan::expose_stan_functions(rstan::stanc(file = "models/v5/test.stan"))
...
# ggsave("models/v5/alpha_beta.png", p_ab, width = 6, height = 3.5, units = "in")
p_ab
...
nt <- 50
n0 <- matrix(c(10*fecundity, 50, 10, 10, 0), 1)
...
# bug: some nonzero p --> a transitions are being inferred
